...
These four works reuse the same four independent sources as the 16:9 set above, now regenerated as complete 3:4 top–bottom canvases. The original photograph remains above; the lower design follows Panel 003's original brief. English copy is generated only from each photograph's content, mood, or implication, and every result was individually reviewed.
...
**Panel 003** is suited to work that must retain the source's essential content while pursuing this visual direction: **Compress direction, connection, pressure, and release into quiet yet tense conceptual black line**. It begins with intended use and desired outcome; layout, size, text, and delivery format follow from those needs.
...
> Use XXD Panel 003 on this image and recommend the best layout and size first.
...
/xxd-panel-003 photo.jpg --mode top-bottom --size auto --text prompt --locale en-GB
...
/xxd-panel-003 photo.jpg --mode design-only --size 3:4 --text prompt --locale en-GB

# Use exact copy without rewriting, translating, or adding a title
/xxd-panel-003 photo.jpg --mode design-only --size 3:4 --copy "Let the light stay" --locale en-GB

# Two modes × three sizes produce six independently composed assets
/xxd-panel-003 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale en-GB

# Apply Panel 003 to a whole directory; shared settings resolve once
/xxd-panel-003 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale en-GB
...
Claude Code users may link the same directory to `~/.claude/skills/xxd-panel-003`. Restart the agent session after installation.
...
- [Chinese runtime adapter](references/xxd-panel-003-prompt.zh-CN.md)
- [English runtime adapter](references/xxd-panel-003-prompt.en.md)
...
**Do not shout a slogan for the photograph; let one line preserve its pressure, passage, and release.**
