...
| --- | --- | --- |
| `top-bottom` | source-adaptive | complete source above, 003 public-issue conceptual line below; both panels retain the source size and split exactly 50/50 |
| `left-right` | source-adaptive | complete source left, 003 public-issue conceptual line right; both panels retain the source size and split exactly 50/50 |
...
Exact user pixels > explicit ratio or destination > source adaptation for ordinary modes. The original `003.md` used a 3:4 creative canvas, but that historical example is not a silent default in the current Skill.
...
- **Independent set:** every device references only the source photograph and may explore a different entry edge, low force zone, crossing route, and release direction.
...
## Copy is only a trace inside the line system
...
Automatic copy distils one very short theme word or phrase from a source-supported relation, pressure, passage, turning point, or release without assigning an issue or advocacy position.
...
The image model creates source-grounded force logic, continuous black route, emergent contour, friction and breaks, justified force marks, pale silent ground, and microtype. `scripts/compose_panel.py` only plans canvases, performs exact 50/50 raster composition, finalises dimensions, and audits results. It never fakes artwork with programmatic drawing.
...
- [Chinese full prompt](references/xxd-panel-003-prompt.zh-CN.md)
- [English full prompt](references/xxd-panel-003-prompt.en.md)
